...
suite(__filename, () => {
  {
        ...
        test("when called, a title must be returned", () => {
          {
            ...
            expected(out).equalTo(`http: options '${params.url}'`);
          }
        });
      }
    });
    suite("handler()", () => {
      {
        const originalOptions = axios.options;
        teardown(() => {
          {
            axios.options = originalOptions;
          }
        });
        test("when called w/o options, {} is used and a response must be returned", async () => {
          {
            ...
            const options = monitor(simulator.fun({
              'resolves': resp
            }));
